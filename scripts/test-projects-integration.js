...
async function testProjectsIntegration() {
  try {
    ...
    const publishedProjects = await Project.find({ status: "Published" }).sort({
      createdAt: -1,
    });
    ...
    console.log(
      `   - Sample project: ${apiResponse.data[0]?.title} (${apiResponse.data[0]?.projectType})`,
    );
    ...
    if (sampleProject) {
      ...
      if (
        ...
        sampleProject.technologies.length > 0
      ) {
        console.log(
          `✅ Technologies array is valid (${sampleProject.technologies.length} items)`,
        );
      } else {
    ...
    await Project.find({ status: "Published" })
      .sort({ createdAt: -1 })
      .limit(10);
    ...
    const projectsWithUrls = publishedProjects.filter(
    ...
    projectsWithUrls.forEach((project) => {
      if (project.websiteUrl && /^https?:\/\/.+/.test(project.websiteUrl))
        validUrls++;
