...
async function finalTestExperienceTimeline() {
  try {
    ...
    timelineData.forEach((entry, index) => {
      ...
      console.log(
        `   - Skills: [${entry.skills.slice(0, 3).join(", ")}${entry.skills.length > 3 ? "..." : ""}]`,
      );
    });
    ...
    const entriesWithoutUrls = timelineData.filter(
      (entry) => !entry.hasValidUrl,
    );
    ...
    if (experiences.length > 0) {
      ...
      console.log("\n🔗 Test URLs:");
      console.log(
        "   • http://localhost:4000/ (scroll to Work Experience section)",
      );
      console.log(
        "   • http://localhost:4000/dashboard/experience (manage experiences)",
      );
