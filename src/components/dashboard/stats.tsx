...
export function DashboardStats() {
  ...
  useEffect(() => {
    async function fetchStats() {
      try {
        const [projectsRes, experienceRes, educationRes, messagesRes] = await Promise.all([
          fetch("/api/projects"),
        ...
        const [projectsData, experienceData, educationData, messagesData] = await Promise.all([
          projectsRes.json(),
          experienceRes.json(),
          educationRes.json(),
          messagesRes.json(),
    ...
    <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
      <motion.div custom={0} initial="hidden" animate="visible" variants={cardVariants}>
        <Card className="bg-[var(--card-background)] border-[var(--card-border-color)] text-[var(--card-headline)]">
          ...
          <CardContent>
            ...
            <p className="text-xs text-[var(--card-paragraph)]">Manage your portfolio projects</p>
          </CardContent>
        </Card>
      </motion.div>
      ...
      <motion.div custom={1} initial="hidden" animate="visible" variants={cardVariants}>
        <Card className="bg-[var(--card-background)] border-[var(--card-border-color)] text-[var(--card-headline)]">
          ...
          <CardContent>
            <div className="text-2xl font-bold">{loading ? "..." : stats.experience}</div>
            <p className="text-xs text-[var(--card-paragraph)]">Work positions listed</p>
          </CardContent>
        </Card>
      </motion.div>
