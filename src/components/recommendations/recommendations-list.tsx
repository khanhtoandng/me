...
import { Edit, Trash2, Calendar, Quote, ExternalLink, Plus } from "lucide-react"
  ...
  if (recommendations.length === 0) {
    return (
      <Card className="bg-[var(--card-background)] border-[var(--card-border-color)] p-8 text-center">
        <h3 className="text-xl font-semibold text-[var(--card-headline)]">No recommendations found</h3>
        <p className="text-[var(--card-paragraph)] mt-2">Start by adding your first recommendation</p>
        ...
        <motion.div key={recommendation._id} variants={itemVariants}>
          <Card className="bg-[var(--card-background)] border-[var(--card-border-color)]">
            <CardHeader className="pb-2">
              <div className="flex items-start justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <CardTitle className="text-lg text-[var(--card-headline)]">{recommendation.name}</CardTitle>
                ...
                <div className="flex gap-2">
                  <Button variant="ghost" size="icon" className="h-8 w-8 text-[var(--paragraph)]" asChild>
                    <Link href={`/dashboard/recommendations/${recommendation._id}/edit`}>
            ...
            <CardContent className="pb-2">
              <div className="flex flex-col gap-4">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <Badge variant="outline" className="border-[var(--card-border-color)]">
                  ...
                  {recommendation.featured && (
                    <Badge className="bg-[var(--tertiary)] text-[var(--button-text)]">Featured</Badge>
                  )}
                </div>
                <Separator className="bg-[var(--card-border-color)]" />
